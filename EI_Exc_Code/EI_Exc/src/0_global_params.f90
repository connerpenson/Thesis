Module global_params
    ...
    integer, parameter :: num_gamma_Q = 8, l_max = 4, n_max = 4, vib_num = 12, num_Q = 19, num_Q_ = 500, skip_en = 4, num_scatE = 85, energy_step_num = 5000
    ...
    real(8), dimension(n_max,0:vib_num-1) ::  chan_ens
    character(len=1), dimension(0:n_max-1) :: state_names
    ...
    real(8), dimension(n_max, 0:vib_num-1, energy_step_num) ::  elec_energies
    ...
    subroutine get_channel_ens
        ...
        do ie = 1, n_max
            ...
            ! cex2 = char(cnj+48)//char(dnj+48)//char(unj+48)
            ! cex2 = char(dnj+48)//char(unj+48)
            open(333, file = "./Energies/" // state_names(ie-1) // "/Target_N2_N2__ElecState001_E.dat", status = "old")
            read(333,*)
            read(333,*)
            read(333,*)
            do iv = 0, vib_num-1
